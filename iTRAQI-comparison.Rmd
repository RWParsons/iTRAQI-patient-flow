...
fx_shorten_path <- function(pu_id_) {
  ...
  origin_row <- 
    ...
    slice(1) |> 
    mutate(NeuroSurgMajor = NA_integer_)
  ...
  if(nrow(centre_rows) == 0) return(select(origin_row, pu_id, TimeWayPoint:FACILITY_NAME_Clean, NeuroSurgMajor))
  
  # print(pu_id_)
  centre_rows <- centre_rows |> 
    select(-NeuroSurgMajor) |>
    mutate(FACILITY_NAME_Clean = as.character(FACILITY_NAME_Clean)) |> 
    left_join(select(df_facilities, FACILITY_NAME_Clean, Latitude, Longitude, NeuroSurgMajor), by = "FACILITY_NAME_Clean") |> 
    mutate(X_COORD = Longitude, Y_COORD = Latitude) |> 
    select(-Latitude, -Longitude) |> 
    slice(1:which.min(NeuroSurgMajor))
  
  bind_rows(origin_row, centre_rows) |> 
    select(pu_id, TimeWayPoint:FACILITY_NAME_Clean, NeuroSurgMajor)
}
