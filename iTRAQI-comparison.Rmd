...
# visualise iTRAQI paths on shiny app
```{r}
shiny::runApp("app")
```
...
df_times |> filter(pu_id == 2195457)

df_times_short |> 
  filter(FACILITY_NAME_Clean == "QCH") |>
  select(pu_id) |> 
  distinct() |> 
  left_join(distinct(df_ages)) |> 
  ggplot(aes(PAT_AGE)) +
  geom_histogram()
...
df_times_short |> 
  ...
  na.omit() |> 
  pull(PAT_AGE) |> 
  (\(x)cbind(Freq=table(x), Cumul=cumsum(table(x)), relative=prop.table(table(x))))() |> 
  as.data.frame()
```
