...
server_map_filters <- function(id, passMap) {
  moduleServer(id, function(input, output, session) {
    ...
    output$freq_table <- renderTable({
      make_freq_table(
        show_ids = group_ids()$show_groups
      )
    })
...
make_freq_table <- function(show_ids) {
  df_op <-
    observed_paths |>
    group_by(pu_id) |>
    slice(1) |>
  ...
  df_filtered_freq <-
    df_op |>
    filter(pu_id %in% clean_show_pu_ids(show_ids)) |>
    group_by(path_category, death_flag, .drop = FALSE) |>
...
clean_show_pu_ids <- function(show_ids) {
  paste0("ID-", unique(str_extract(show_ids, "[0-9]*$")))
}
